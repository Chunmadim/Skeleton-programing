...
class Dastan:
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                ...
                if Choice == 9:
                    ...
                    self.__DisplayState()
            piece = self._CurrentPlayer.GetMoveOption(Choice - 1)
            print(piece.GetName())
            ...
            SquareIsValid = False
            if piece.GetName() == "rush":
                FinishSquareReference = StartSquareReference
                if self._CurrentPlayer.SameAs(self._Players[0]):
                    while not SquareIsValid and self.__CheckSquareInBounds(FinishSquareReference):
                        FinishSquareReference += 10
                        square = self._Board[self.__GetIndexOfSquare(FinishSquareReference)].GetPieceInSquare()
                        if square != None:
                            player = square.GetBelongsTo()
                            if player.SameAs(self._Players[0]):
                                FinishSquareReference -= 10
                            SquareIsValid = self.__CheckSquareIsValid(FinishSquareReference, False)
                    if not self.__CheckSquareInBounds(FinishSquareReference):
                        FinishSquareReference -= 10
                    ...
                    while not SquareIsValid and self.__CheckSquareInBounds(FinishSquareReference):
                        ...
                        if square != None:
                            player = square.GetBelongsTo()
                            if player.SameAs(self._Players[1]):
                                FinishSquareReference += 10
                            SquareIsValid =  self.__CheckSquareIsValid(FinishSquareReference, False)
                    if not self.__CheckSquareInBounds(FinishSquareReference):
                        FinishSquareReference -= 10
    ...
    def __CreateMoveOptionOffer(self):
        self._MoveOptionOffer.append("rush")
    ...
    def __CreateRushMoveOption(self, Direction):
        NewMoveOption = MoveOption("rush")
        for i in range(self._NoOfRows):
            NewMove = Move(i * Direction,0) 
            NewMoveOption.AddToPossibleMoves(NewMove)
        return NewMoveOption
    ...
    def __CreateMoveOption(self, Name, Direction):
        if Name == "chowkidar":
            ...
            return self.__CreateRushMoveOption(Direction)
        else:
            return self.__CreateCuirassierMoveOption(Direction)

    def __CreateMoveOptions(self):
        self._Players[0].AddToMoveOptionQueue(self.__CreateMoveOption("rush",1))
        ...
        self._Players[1].AddToMoveOptionQueue(self.__CreateMoveOption("rush",-1))
...
class Player:
    ...
    def GetMoveOption(self,Choice):
        return self.__Queue.GetMoveOptionInPosition(Choice)
