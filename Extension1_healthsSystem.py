...
class Dastan:
    ...
    def __ManhatanDistanceDamage(self,StartSquareReference,FinishSquareReference,pos):
        StartRow = (StartSquareReference // 10)
        StartCol = StartSquareReference % 10
        FinishRow = FinishSquareReference // 10
        FinishCol = FinishSquareReference % 10
        return (abs(StartRow - FinishRow) + abs(StartCol - FinishCol)) + pos
    ...
    def __DisplayBoard(self):
        ...
        for Row in range(1, self._NoOfRows + 1):
            ...
            for Column in range(1, self._NoOfColumns + 1):
                ...
                if self._Board[Index].GetAttackingPiece() is not None:
                    print("|" + self._Board[Index].GetAttackingPiece().GetSymbol(),end="")
                else:
    ...
    def __CheckSquareIsValid(self, SquareReference, StartSquare):
        ...
        if PieceInSquare is None:
            ...
            if StartSquare:
                ...
                return False
        elif self._Board[self.__GetIndexOfSquare(SquareReference)].GetAttackingPiece() is not None:
            if self._CurrentPlayer.SameAs(self._Board[self.__GetIndexOfSquare(SquareReference)].GetAttackingPiece().GetBelongsTo()):
                if StartSquare:
                    return True
        else:
            if StartSquare:
                ...
                if self._Board[self.__GetIndexOfSquare(SquareReference)].GetAttackingPiece() is None:
                    return True
                else:
                    return False
    ...
    def __UpdatePlayerScore(self):
        self._CurrentPlayer.ChangeScore(self.__GetPointsForOccupancyByPlayer(self._CurrentPlayer))
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            for i in self._Board:
                if i.GetPieceInSquare() is not None:
                    print(i.GetPieceInSquare().GetHealths())
            self.__DisplayState()
            SquareIsValid = False
            Choice = 0
            ...
            if MoveLegal:
                if self._Board[self.__GetIndexOfSquare(FinishSquareReference)].GetPieceInSquare() is not None:
                    Damage = self.__ManhatanDistanceDamage(StartSquareReference,FinishSquareReference,Choice)
                    self._Board[self.__GetIndexOfSquare(StartSquareReference)].GetPieceInSquare().SetDamage(Damage)
                ...
                self.__UpdatePlayerScore()
                print("New score: " + str(self._CurrentPlayer.GetScore()) + "\n")
            for Square in self._Board:
                if Square.GetAttackingPiece() is not None and Square.GetPieceInSquare() is not None:
                    healths = Square.GetPieceInSquare().GetHealths() - Square.GetAttackingPiece().GetDamage()
                    Square.GetPieceInSquare().SetHealths(healths)
                    if Square.GetPieceInSquare().GetHealths() <= 0:
                        PointsForPieceCapture = Square.GetPieceInSquare().GetPointsIfCaptured()
                        AttackingPieceOwner = Square.GetAttackingPiece().GetBelongsTo()
                        AttackingPieceOwner.ChangeScore(PointsForPieceCapture)
                        Square.Change()
                if Square.GetAttackingPiece() is not None and Square.GetPieceInSquare() is None:
                    Square.Change()
    ...
    def __UpdateBoard(self, StartSquareReference, FinishSquareReference):
        if self._Board[self.__GetIndexOfSquare(StartSquareReference)].GetPieceInSquare().GetBelongsTo().SameAs(self._CurrentPlayer):
            if self._Board[self.__GetIndexOfSquare(FinishSquareReference)].GetPieceInSquare() is not None:
                self._Board[self.__GetIndexOfSquare(FinishSquareReference)].SetAttackingPiece(self._Board[self.__GetIndexOfSquare(StartSquareReference)].RemovePiece())
            else:
        ...
        elif self._Board[self.__GetIndexOfSquare(StartSquareReference)].GetAttackingPiece() is not None and self._Board[self.__GetIndexOfSquare(StartSquareReference)].GetAttackingPiece().GetBelongsTo().SameAs(self._CurrentPlayer):
            if self._Board[self.__GetIndexOfSquare(FinishSquareReference)].GetPieceInSquare() is None:
                self._Board[self.__GetIndexOfSquare(FinishSquareReference)].SetPiece(self._Board[self.__GetIndexOfSquare(StartSquareReference)].RemoveAttackingPiece())
            else:
                self._Board[self.__GetIndexOfSquare(FinishSquareReference)].SetAttackingPiece(self._Board[self.__GetIndexOfSquare(StartSquareReference)].RemoveAttackingPiece())
        else:
            self._Board[self.__GetIndexOfSquare(FinishSquareReference)].SetPiece(self._Board[self.__GetIndexOfSquare(StartSquareReference)].RemovePiece())
    ...
    def __CreatePieces(self, NoOfPieces):
        ...
        CurrentPiece = Piece("piece", self._Players[1], 1, '"')
        self._Board[self.__GetIndexOfSquare(32)].SetPiece(CurrentPiece)
...
class Square:
    def __init__(self):
        ...
        self._Symbol = " "
        self._AttackingPiece = None

    def Change(self):
        self._PieceInSquare = self._AttackingPiece
        self._AttackingPiece = None

    def GetAttackingPiece(self):
        return self._AttackingPiece

    def SetAttackingPiece(self,P):
        self._AttackingPiece = P

    def RemoveAttackingPiece(self):
        PieceToReturn = self._AttackingPiece
        self._AttackingPiece = None
